...
{
    public partial class IndividualTrainingPaymentWindow : Window
    {
        private readonly Models.Trainer _trainer;
        private readonly IndividualTrainingSlot _slot;
        ...
        public IndividualTraining NewTraining { get; private set; }

        public IndividualTrainingPaymentWindow(Models.Trainer trainer, IndividualTrainingSlot slot, Models.Client client, TrainingService trainingService)
        {
            InitializeComponent();
            _trainer = trainer;
            _slot = slot;
            ...
            _finalPrice = _trainer.IndividualTrainingPrice;

            TrainingTypeText.Text = "Индивидуальная тренировка";
            TrainerNameText.Text = $"{trainer.FullName}";
            TrainingDateTimeText.Text = $"{slot.TrainingDate:dd.MM.yyyy} | {slot.StartTime:hh\\:mm} - {slot.EndTime:hh\\:mm}";
        ...
        {
            _finalPrice = Math.Max(0, _trainer.IndividualTrainingPrice - _usedBonusPoints);
        ...
        private async void PayButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                using (var context = new AppDbContext())
                {
                    ...
                    var existingSlot = context.IndividualTrainings.FirstOrDefault(it => 
                        it.TrainerId == _trainer.TrainerId && 
                        it.TrainingDate.Date == _slot.TrainingDate.Date && 
                        it.StartTime == _slot.StartTime && 
                        it.ClientId == null && 
                        it.IsActive);

                    if (existingSlot != null)
                    {
                        existingSlot.ClientId = _client.ClientId;
                        existingSlot.Price = _finalPrice;
                        existingSlot.CreatedAt = DateTime.UtcNow;
                    }
                    else
                    {
                        var newTraining = new IndividualTraining
                        {
                            ClientId = _client.ClientId,
                            TrainerId = _trainer.TrainerId,
                            TrainingDate = _slot.TrainingDate,
                        ...
                        context.IndividualTrainings.Add(newTraining);
                    }

                    context.TrainerNotifications.Add(new TrainerNotification
                    {
                        TrainerId = _trainer.TrainerId,
                        Message = $"Клиент {_client.FullName} записался на индивидуальную тренировку ({_slot.TrainingDate:dd.MM.yyyy} в {_slot.StartTime:hh\\:mm})",
                        IsRead = false,
                    ...
                    var savedTraining = await context.IndividualTrainings
                        .Include(t => t.Trainer)
                        .Include(t => t.Client)
                        .FirstOrDefaultAsync(t => t.TrainerId == _trainer.TrainerId && t.TrainingDate == _slot.TrainingDate && t.StartTime == _slot.StartTime);
                    ...
                    if (savedTraining != null)
                    {
                        ...
                        await SaveReceiptWithDialog(savedTraining, bonusCardInDb, bonusCardInDb?.PointsBalance ?? 0);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                var box = MessageBoxManager.GetMessageBoxStandard("Ошибка", $"Произошла ошибка при оплате: {ex.Message}", ButtonEnum.Ok);
                await box.ShowWindowDialogAsync(this);
            }
        }

        private async Task SaveReceiptWithDialog(IndividualTraining training, BonusCard bonusCard, int finalBalance)
        {
            try
            {
                int earnedPoints = (int)Math.Floor(_finalPrice * 0.02m);
                string bonusCardInfo = bonusCard != null ? $"Списано: {_usedBonusPoints} Б\nНачислено: {earnedPoints} Б\nОстаток: {finalBalance} Б\n" : "";
                string statusText = training.IsActive ? "Оплачено" : "Отменена";
                string receiptText = $"ЧЕК ОБ ОПЛАТЕ\n\nДата: {DateTime.Now:dd.MM.yyyy HH:mm}\nУслуга: Индивидуальная тренировка\nКлиент: {_client.FullName}\nТренер: {_trainer.FullName}\nВремя: {training.TrainingDate:dd.MM.yyyy} ({training.StartTime:hh\\:mm}-{training.EndTime:hh\\:mm})\nСтатус: {statusText}\n\nК оплате: {_finalPrice:N0} ₽\n{bonusCardInfo}";
