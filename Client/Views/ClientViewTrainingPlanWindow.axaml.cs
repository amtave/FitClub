...
namespace FitClub.Client.Views
{
    public partial class ClientViewTrainingPlanWindow : Window
    {
        ...
        {
            GoalInfoText.Text = _plan.Goal?.Name ?? "Не указана";
            TrainerInfoText.Text = _plan.Trainer?.FullName ?? "Неизвестно";
            CreatedDateText.Text = $"Создан: {_plan.CreatedDate:dd.MM.yyyy}";
            NotesText.Text = string.IsNullOrEmpty(_plan.Notes) ? "Тренер не оставил примечаний к этому плану." : _plan.Notes;

            StatusText.Text = $"{_plan.CompletedDays} из 12 дней заполнено";
