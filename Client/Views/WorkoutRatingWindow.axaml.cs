    ...
    public partial class WorkoutRatingWindow : Window
    {
        ...
        private async void Submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var clientEmail = UserSession.CurrentUserEmail;
                var client = db.Clients.FirstOrDefault(c => c.Email == clientEmail);
                string clientName = client != null ? client.FullName : "Клиент";

                if (_workoutItem.WorkoutType == "Group" && _workoutItem.OriginalObject is TrainingBooking gb)
                {
                    var booking = db.TrainingBookings.Include(b => b.TrainingSchedule).FirstOrDefault(b => b.BookingId == gb.BookingId);
                    if (booking != null)
                    {
                        ...
                        if (booking.TrainingSchedule != null)
                        {
                            ...
                            {
                                TrainerId = booking.TrainingSchedule.TrainerId,
                                Message = $"Клиент {clientName} оставил отзыв о вашей групповой тренировке (Оценка: {_currentRating} ★).",
                ...
                else if (_workoutItem.WorkoutType == "Individual" && _workoutItem.OriginalObject is IndividualTraining it)
                {
                    ...
                    if (training != null)
                    {
                        ...
                        db.TrainerNotifications.Add(new TrainerNotification
                        {
                            ...
                            Message = $"Клиент {clientName} оставил отзыв о вашей индивидуальной тренировке (Оценка: {_currentRating} ★).",
                            CreatedAt = DateTime.Now,
                            IsRead = false
                        });
                    }
