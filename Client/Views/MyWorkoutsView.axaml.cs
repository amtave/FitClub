...
namespace FitClub.Views
{
    ...
    public partial class MyWorkoutsView : UserControl
    {
        ...
        private TrainingPlan _clientPlan;
        private bool _hasIndividualSubscription;
        ...
        private void CheckClientPlanStatus()
        {
            if (_currentClient == null) return;

            _hasIndividualSubscription = _context.ClientSubscriptions.Any(cs => 
                cs.ClientId == _currentClient.ClientId && 
                cs.IsActive && 
                cs.EndDate >= DateTime.Today &&
                cs.IndividualTrainerId != null);

            if (!_hasIndividualSubscription)
            {
                NoSubscriptionBanner.IsVisible = true;
                PlanInPreparationBanner.IsVisible = false;
                PlanButton.IsEnabled = false;
                PlanButton.Foreground = Brush.Parse("#BDC3C7");
                ToolTip.SetTip(PlanButtonBorder, "Доступно при наличии индивидуального абонемента");
                return;
            }

            _clientPlan = _context.TrainingPlans
                .Include(tp => tp.TrainingPlanDays)
                .FirstOrDefault(tp => tp.ClientId == _currentClient.ClientId && tp.IsActive);

            if (_clientPlan == null || _clientPlan.CompletedDays == 0)
            {
                NoSubscriptionBanner.IsVisible = false;
                PlanInPreparationBanner.IsVisible = true;
                PlanButton.IsEnabled = false;
                PlanButton.Foreground = Brush.Parse("#BDC3C7");
                ToolTip.SetTip(PlanButtonBorder, "План находится в разработке у тренера");
            }
            else
            {
                NoSubscriptionBanner.IsVisible = false;
                ...
                PlanButton.Foreground = Brush.Parse("#3498DB");
                ToolTip.SetTip(PlanButtonBorder, null);
            }
        }
        ...
        private async void OpenPlan_Click(object sender, RoutedEventArgs e)
        {
            if (_clientPlan != null)
            {
                var viewWindow = new ClientViewTrainingPlanWindow(_clientPlan);
                ...
                if (confirm == ButtonResult.Yes)
                {
                    try
                    {
                        ...
                        if (item.WorkoutType == "Group" && item.OriginalObject is TrainingBooking gb)
                        {
                            ...
                            var s = db.TrainingSchedules.Include(ts => ts.GroupTraining).FirstOrDefault(ts => ts.ScheduleId == gb.ScheduleId);
                            
                            if (s != null && s.CurrentParticipants > 0)
                            {
                                ...
                                db.TrainerNotifications.Add(new TrainerNotification
                                {
                                    ...
                                    Message = $"Клиент {_currentClient.FullName} отменил запись на групповую тренировку «{s.GroupTraining?.Name}» ({s.TrainingDate:dd.MM.yyyy} в {s.TrainingTime:hh\\:mm})",
                                    IsRead = false,
                        ...
                        else if (item.WorkoutType == "Individual" && item.OriginalObject is IndividualTraining it)
                        {
                            ...
                            { 
                                ...
                                db.TrainerNotifications.Add(new TrainerNotification
                                {
                                    ...
                                    Message = $"Клиент {_currentClient.FullName} отменил индивидуальную тренировку ({t.TrainingDate:dd.MM.yyyy} в {t.StartTime:hh\\:mm})",
                                    IsRead = false,
                                    CreatedAt = DateTime.Now
                                });
