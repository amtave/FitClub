...
{
    public partial class TrainerPaymentTrainingWindow : Window
    {
        ...
        public TrainingBooking NewBooking { get; private set; }

        public TrainerPaymentTrainingWindow(GroupTraining training, TrainingSchedule schedule, 
                                          Models.Client client, TrainingService trainingService,
                                          Models.Trainer trainer)
        {
            ...
            // Заполняем информацию о тренировке
            TrainingNameText.Text = training.Name;
            TrainerNameText.Text = $"Тренер: {trainer.FullName}";
            TrainingDateTimeText.Text = $"{schedule.DateFormatted} в {schedule.TimeFormatted}";
            TrainingPriceText.Text = $"Стоимость: {training.Price:0.00} ₽";
        ...
        {
            TrainingPriceText.Text = $"Стоимость: {_training.Price:0.00} ₽";
        ...
        private async void PayButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                Console.WriteLine($"=== ОПЛАТА ТРЕНЕРСКОЙ ТРЕНИРОВКИ ===");
                Console.WriteLine($"Клиент ID: {_client.ClientId}");
                Console.WriteLine($"Тренировка: {_training.Name} (ID: {_training.TrainingId})");
                Console.WriteLine($"Тренер: {_trainer.FullName} (ID: {_trainer.TrainerId})");
                Console.WriteLine($"Расписание ID: {_schedule.ScheduleId}");
                Console.WriteLine($"Дата: {_schedule.TrainingDate:dd.MM.yyyy}");
                Console.WriteLine($"Время: {_schedule.TrainingTime:hh\\:mm}");
                Console.WriteLine($"Базовая стоимость: {_training.Price:0.00} ₽");
                ...
                using (var context = new AppDbContext())
                {
                    try
                    {
                        // Загружаем актуальную бонусную карту из БД
                    ...
                    {
                        Console.WriteLine($"Ошибка обработки бонусных баллов: {ex.Message}");
                    }
                }
                ...
                try
                {
                    // Проверяем данные перед созданием записи
                    if (_client?.ClientId == 0 || _schedule?.ScheduleId == 0 || _training?.TrainingId == 0)
                    {
                        await ShowMessageAsync("Ошибка", "Неверные данные для записи");
                    ...
                    bool alreadyBooked = _trainingService.CheckIfClientAlreadyBooked(_client.ClientId, _schedule.ScheduleId);
                    if (alreadyBooked)
                    {
                        await ShowMessageAsync("Ошибка", "Вы уже записаны на эту тренировку!");
                        PayButton.IsVisible = true;
                        LoadingBorder.IsVisible = false;
                        return;
                    }
                    ...
                    bool bookingSuccess = _trainingService.CreateTrainingBooking(booking);

                    if (!bookingSuccess)
                    {
                        await ShowMessageAsync("Ошибка", "Не удалось создать запись в базе данных");
                        PayButton.IsVisible = true;
                        LoadingBorder.IsVisible = false;
                        return;
                    }

                    // Получаем созданную запись для проверки
                    NewBooking = _trainingService.GetLastBooking(_client.ClientId, _schedule.ScheduleId);
                    
                    if (NewBooking == null || NewBooking.BookingId == 0)
                    {
                        Console.WriteLine("⚠️ Запись создана, но не найдена в базе данных");
                    }
                    else
                    {
                        Console.WriteLine($"✅ Запись найдена в БД, ID: {NewBooking.BookingId}");
                    }
                }
                catch (Exception dbEx)
                {
                    await ShowMessageAsync("Ошибка БД", $"Ошибка при создании записи: {dbEx.Message}");
                    PayButton.IsVisible = true;
                    LoadingBorder.IsVisible = false;
                    return;
                }
                ...
                Console.WriteLine($"=== ОПЛАТА ТРЕНЕРСКОЙ ТРЕНИРОВКИ УСПЕШНО ЗАВЕРШЕНА ===");
            }
            catch (Exception ex)
            {
                LoadingBorder.IsVisible = false;
                ...
                Console.WriteLine($"❌ ОШИБКА ОПЛАТЫ ТРЕНЕРСКОЙ ТРЕНИРОВКИ: {ex.Message}");
        ...
        private async Task SaveReceiptWithDialog()
        {
            try
            {
                decimal finalPrice = _finalPrice;
                int earnedPoints = (int)Math.Floor(_finalPrice * 0.02m);
                int finalBalance = 0;
                ...
                if (_bonusCard != null)
                {
                    ...
                    bonusInfo = $"Бонусная карта: {_bonusCard.FormattedCardNumber}\n" +
                               $"Списано баллов: {_usedBonusPoints}\n" +
                               $"Начислено бонусных баллов: {earnedPoints} (2% от {finalPrice:0.00} ₽)\n" +
                ...
                string receiptText = $"Чек об оплате групповой тренировки\n" +
                                    $"Дата: {DateTime.Now:dd.MM.yyyy HH:mm}\n" +
                                    $"Клиент: {_client.FullName}\n" +
                                    $"Тренировка: {_training.Name}\n" +
                                    $"Тренер: {_trainer.FullName}\n" +
                                    $"Дата: {_schedule.TrainingDate:dd.MM.yyyy}\n" +
                                    $"Время: {_schedule.TrainingTime:hh\\:mm}\n" +
                                    $"Базовая стоимость: {_training.Price:0.00} ₽\n" +
                                    $"Итоговая стоимость: {finalPrice:0.00} ₽\n" +
                                    $"{bonusInfo}" +
                                    $"Номер чека: {NewBooking?.BookingId ?? 0}";

                // Показываем чек в сообщении
